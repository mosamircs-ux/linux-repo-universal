...
def build_squashfs(rootfs_dir: str, output_squashfs: str, epoch: Optional[int] = None, comp: str = "zstd") -> str:
    ...
    if mksquashfs_bin:
        # Build mksquashfs command with deterministic reproducibility flags
        cmd = [
            mksquashfs_bin,
            rootfs_dir,
            output_squashfs,
        ...
        # Test if -mkfs-time / -reproducible are supported by host mksquashfs version
        env = os.environ.copy()
        env["SOURCE_DATE_EPOCH"] = str(epoch)
        
        # Try full reproducible flags first
        ...
        # Write deterministic squashfs container format for clean test/build environments
    ...
    # Clamp output file timestamp to epoch
    try:
